import React, { useState, useCallback } from 'react'
import { useDispatch, useSelector } from 'react-redux'

// Selectors
import { getDesks } from '../../store/desk/selectors'

// Material-ui
import Fab from '@material-ui/core/Fab'
import AddIcon from '@material-ui/icons/Add'
import IconButton from '@material-ui/core/IconButton';
import UpdateIcon from '@material-ui/icons/Update';
import DeleteIcon from '@material-ui/icons/Delete';
import Box from '@material-ui/core/Box'
import { makeStyles } from '@material-ui/core/styles'
import Table from '@material-ui/core/Table';
import TableBody from '@material-ui/core/TableBody';
import TableCell from '@material-ui/core/TableCell';
import TableContainer from '@material-ui/core/TableContainer';
import TableHead from '@material-ui/core/TableHead';
import TableRow from '@material-ui/core/TableRow';
import Paper from '@material-ui/core/Paper';

// Components
import Modal from './Modal'

// reducers
import { addDesk, deleteDesk, updateDesk } from '../../store/desk/reducers'

const useStyles = makeStyles(theme => ({
    root: {
        width: '100%'
    },
    tableRoot: {
        width: '100%'
    },
    fab: {
        position: 'absolute',
        bottom: theme.spacing(2),
        right: theme.spacing(2),
    },
}))

const DEFAULT_OPENMODAL = {
    open: false,
    isUpdate: false,
    data: null
}

const Desks = () => {
    const classes = useStyles()
    const dispatch = useDispatch()

    // Selectors
    const desks = useSelector(getDesks)

    // State
    const [openModal, setOpenModal] = useState(DEFAULT_OPENMODAL)
    const [fields, setFields] = useState({
        deskId: null
    })

    const handleUpdateModal = (values) => {
        setOpenModal({
            open: true,
            isUpdate: true,
            data: {
                ...values
            }
        })
    }

    const listDesks = () => {
        return desks.map((desk, idx) => (
            <TableRow key={`${desk.deskId}-${idx}`}>
                <TableCell component="th" scope="row">
                    {desk.deskId}
                </TableCell>
                <TableCell>
                    <IconButton onClick={() => handleUpdateModal(desk)}>
                        <UpdateIcon />
                    </IconButton>
                    <IconButton onClick={() => removeDesk(desk.id)}>
                        <DeleteIcon />
                    </IconButton>
                </TableCell>
            </TableRow>
        ))
    }

    const handleModal = useCallback(() => {
        setFields({
            deskId: null
        })
        setOpenModal({
            open: !openModal.open,
            isUpdate: false
        })
    }, [openModal])

    const handleDeskIdField = useCallback((e) => {
        if (!openModal.isUpdate)
            setFields({
                ...fields,
                deskId: e.target.value
            })
        else
            setOpenModal({
                ...openModal,
                data: {
                    ...openModal.data,
                    deskId: e.target.value
                }
            })
    }, [fields, openModal])

    const addNewDesk = useCallback(() => {
        dispatch(addDesk(fields.deskId))
        setOpenModal(DEFAULT_OPENMODAL)
    }, [fields.deskId, dispatch])

    const removeDesk = useCallback((id) => {
        dispatch(deleteDesk(id))
    }, [dispatch])

    const handleUpdateDesk = useCallback(() => {
        dispatch(updateDesk(openModal.data))
        setOpenModal(DEFAULT_OPENMODAL)
    }, [dispatch, openModal])


    return (<Box mt={3} className={classes.root} display="flex" flexDirection="column">
        <TableContainer component={Paper}>
            <Table className={classes.table} aria-label="simple table">
                <TableHead>
                    <TableRow>
                        <TableCell>Desk ID</TableCell>
                        <TableCell>Actions</TableCell>
                    </TableRow>
                </TableHead>
                <TableBody>
                    {listDesks()}
                </TableBody>
            </Table>
        </TableContainer>

        <Modal {...{
            handleDeskIdField,
            handleModal,
            openModal,
            handleUpdateDesk,
            addNewDesk,
            setOpenModal
        }}
        />
        <Fab onClick={() => setOpenModal({
            open: true,
            isUpdate: false
        })} className={classes.fab} color="primary" aria-label="add">
            <AddIcon />
        </Fab>
    </Box >)
}

export default Desks